...
You have a small e-commerce app, where client during checkout need to fill address form, but also can
...
It works, but it's not DRY, hard to read and if would like to make ui more fancy for address fields. You would need to change it in multiple places.
We can extract address fields to a separate component and as documentation of `react-hook-form` we can use [`useFormContext`](https://react-hook-form.com/docs/useformcontext).
...
Ok. It looks better now, but here's a problem - we cannot reuse `AddressFields` in `ProfileForm` because it's strongly typed to `CheckoutForm`.
We can fix by more generic type of context (which hard to type), but what if someone would like to use `AddressFields` in a form flat structure or more nested in components tree?

<H level={3}> My solution</H>

First of all, I try to think how to make `AddressFields` "knowing" nothing about the form structure. I can pass `register` function as a prop
...
That simple! Notice with if `AddressFields` would change in the future, form itself doesn't have to be adjusted.

<H level={3}>Sum up!</H>

In this article, we explore how to reuse groups of form fields in a React application using TypeScript
...
2. Make component generic and independent of form structure by passing custom register function as a prop.
